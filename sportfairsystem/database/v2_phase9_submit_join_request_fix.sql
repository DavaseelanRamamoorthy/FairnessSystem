-- V2 Phase 9 submit join request RPC ambiguity fix
--
-- Purpose:
-- 1. Fix PL/pgSQL output variable shadowing in submit_team_join_request_by_code.
-- 2. Ensure pending-request lookup and insert RETURNING clauses use explicit table qualification.
...
begin
  ...
  from public.teams
  where public.teams.join_code = normalized_team_join_code;
  ...
  where public.team_join_requests.requester_user_id = current_user_id
    and public.team_join_requests.status = 'pending'
  order by public.team_join_requests.requested_at desc
  limit 1;
  ...
  returning
    public.team_join_requests.id,
    public.team_join_requests.team_id,
    public.team_join_requests.requester_name,
    public.team_join_requests.status,
    public.team_join_requests.requested_at
