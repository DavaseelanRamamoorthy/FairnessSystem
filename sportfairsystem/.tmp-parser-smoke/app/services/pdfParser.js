...
function parseInningsBlocks(fullText) {
    return fullText.match(/([A-Za-z\s().&]+?\s+\d+\/\d+\s+\(\d+(\.\d+)?\s+Ov\)\s+\(1st Innings\)[\s\S]+?)(?=[A-Za-z\s().&]+?\s+\d+\/\d+\s+\(\d+(\.\d+)?\s+Ov\)\s+\(1st Innings\)|Match Officials|$)/g) ?? [];
}
...
async function parseMatchFromBase64(base64Data, currentTeamName) {
    ...
    for (let i = 1; i <= pdf.numPages; i += 1) {
        ...
        const pageText = content.items
            ...
            .join(" ");
        pageTexts.push((0, matchTextNormalization_1.normalizeMatchWhitespace)(pageText));
    }
    ...
    const matchTitle = titleMatch ? (0, matchTextNormalization_1.normalizeMatchWhitespace)(titleMatch[1]) : null;
    ...
    let winner = null;
    if (resultMatch) {
        winner = (0, matchTextNormalization_1.normalizeMatchWhitespace)(resultMatch[1]);
    }
    const resultSummary = resultSummaryMatch
        ? (0, matchTextNormalization_1.normalizeMatchWhitespace)(resultSummaryMatch[1])
        : null;
    ...
    const inningsSummaries = parseInningsBlocks(fullText).map((block, index) => {
        const teamMatch = block.match(/^([A-Za-z\s().&]+?)\s+\d+\/\d+\s+\(\d+(\.\d+)?\s+Ov\)/i);
        const teamName = teamMatch ? (0, matchTextNormalization_1.normalizeMatchWhitespace)(teamMatch[1]) : null;
        ...
        const playing11 = Array.from(new Set([
            ...squadNames,
            ...battingStats.map((player) => (0, matchTextNormalization_1.stripNameAnnotations)(player.player_name)),
            ...pendingBatters
        ]));
        return {
